...
import com.dangong.oksan.util.progress.ProgressHelper;
...
public class UpLoadUtils {
    ...
    private static Request getRequest(String url, List<String> fileNames, ProgressListener uiProgressRequestListener) {
        ...
        builder.url(url)
                .post(ProgressHelper.addProgressRequestListener(
                        getRequestBody(fileNames),
                        uiProgressRequestListener));

        builder .addHeader("Authorization", "Bearer " + Constants.loginInfo.getToken());
    ...
    private static RequestBody getRequestBody(List<String> fileNames) {
        ...
        builder.addFormDataPart("phone", Constants.loginInfo.getPhone());
        return builder.build(); //根据Builder创建请求
    }
    ...
    public static void downloadAndSaveFile(final Activity activity, String downloadUrl, final String savePath, UIProgressListener uiProgressResponseListener) {
        //包装Response使其支持进度回调
        ProgressHelper.addProgressResponseListener(getOkHttpClientInstance(), uiProgressResponseListener, savePath)
                ...
                .enqueue(new Callback() {
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        Log.i("TAG", "服务器响应成功");
                          //在本地保存文件
                        saveDownloadFile(response, savePath);
                    }
                });
    }
